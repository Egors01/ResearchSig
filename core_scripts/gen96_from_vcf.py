...
def gen96_matrix_from_filtered_vcf(filt_vcf_names_list, run_id=None):
    ...
    logger = Logger(source_name='table_generator',
                    msg='Launched table creation', run_id=run_id)
    for filename in filt_vcf_names_list:
        ...
        # vcf_data['NSUBS'] = vcf_data['REF'].str[:] + '>' + vcf_data['ALT'].str[                                               :]
        # vcf_data.groupby(['SUBS', 'Context']).size().reset_index(name=name)
        # name = filename.split(".filt.vcf")[0].split("/")[1]
        name = filename.split("/")[-1].split('.filt.vcf')[0]
        print(name)
        df = vcf_data.groupby(['SUBS', 'Context']).size().reset_index(
            name=name)
...
def gen192_matrix_from_filtered_vcf(filt_vcf_names_list, run_id=None):
    result = pd.DataFrame()
    MATRIX_NAME = os.path.join(MATRICES_PATH,
                               '192_matrix.run_' + run_id + '.csv')
    logger = Logger(source_name='table_generator',
                    msg='Launched table creation', run_id=run_id)
    for filename in filt_vcf_names_list:
        print('\n192 \n',filename)
